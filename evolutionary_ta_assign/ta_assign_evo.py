"""
Using evolutionary programming to assign ta's to lab sections
Jeremiah Payeur, Sahana Dhar, Madelyn Redick
ta_assign_evo.py

Note: Code from Professor Rachlin's lecture was used to aid with this assignment
"""
...
"""These three functions were taken from Professor Rachlin's lecture"""
...
def add_random(L, W, max):
    """Add people random assignments to places where they are not currently assigned"""

    return np.array([L[i] if 1 in L[i] else [L[i][j] == 1 if j in rnd.sample([k for k in range(len(W[i]))
                                                                              if W[i][k] == 'W' or W[i][k] == 'P'],
                                                                             max[i]) else 0 for j in range(len(L[i]))]
...
def main():
    ...
    E.add_agent('add_random', add_random, W, max, k=1)
