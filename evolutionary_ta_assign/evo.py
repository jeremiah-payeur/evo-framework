...
import time
"""
Evolutionary Framework developed in class with minor edits
"""
...
class Evo:
    ...
    def run_agent(self, name):
        ...
        # this line right here will return a list based on the number of solutions we want to try
    ...
    def evolve(self, nds, n=30, dom=250):
        ...
        sols = 0
        while time.time() - start < length/20:
            self.run_agent('max_p')
            sols+=1
            if sols % dom==0:
                self.remove_dominated()
        while time.time() - start < length/7:
            self.run_agent('swap')
            sols+=1
            if sols % dom==0:
                self.remove_dominated()
        while time.time() - start < length/4:
            self.run_agent('add_random')
            sols+=1
            if sols % dom==0:
                self.remove_dominated()
